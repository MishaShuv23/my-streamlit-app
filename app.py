...
# Относительный путь к данным — работает везде!
CSV_FILE = os.path.join(os.path.dirname(__file__), "data", "moscow_places.csv")

# Константы колонок (дублируем для ясности)
COL_NAME = 'name'
...
def main():
    ...
    with st.form(key="search_form"):
        ...
        with col1:
            selected_categories = st.multiselect("Тип заведения", options=all_categories, default=[])
            selected_districts = st.multiselect("Округ", options=all_districts, default=[])
    ...
    if submit_button:
        ...
        if result_df.empty:
            ...
            for _, row in result_df.iterrows():
                ...
                has_location = pd.notna(row.get(COL_LAT)) and pd.notna(row.get(COL_LNG))
                map_badge = (
